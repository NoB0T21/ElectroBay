...
const ProductForm = () => {
  ...
  return (
    <>
        <form onSubmit={handleSubmit} className="flex flex-col items-center gap-4 p-1 rounded-md w-full lg:w-2/3">
            <div className="gap-3 grid grid-cols-2 md:grid-cols-5 w-full">
                { 
                    num.map((nu,index)=>(
                        <div key={nu} className="size-15 md:size-20">
            ...
            <div className="relative w-full">
                ...
                <input name='product name' type="text" value={formData.name} onChange={(e) => {setFormData({...formData, name: e.target.value})}}required 
            ...
            <div className="gap-2 grid grid-cols-1 md:grid-cols-3 w-full">
                ...
                className="bg-zinc-800 p-2 border-0 focus:border-indigo-500 rounded-md outline-none w-2/3 h-10 text-white"
                >
            ...
            <div className="w-2/3">
                <button type="submit" className="bg-indigo-600 hover:bg-indigo-700 p-2 rounded-md w-full font-semibold text-md">{loading? <PulseLoader color="#fff"/>:'ADD'}</button>
            </div>
        </form>
