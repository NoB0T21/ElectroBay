...
const Page = async ({searchParams}:SearchParamProps) => {
  ...
  let products:any
  if(sorts){
      products = await api.get(`/product/mobiles?sort=${sorts}`,{
      ...
      products = await api.get('/product/mobiles',{
      withCredentials:true,
      headers:{
        Authorization: `Bearer ${token}`,
      }
  ...
  return (
    <div className='w-full'>
      <div className='flex flex-col justify-center gap-4 px-4 md:px-8 py-8 w-full'>
        <h1 className='font-bold text-slate-800 text-3xl md:text-4xl lg:text-5xl'>Mobiles</h1>
